...
t_crit = DATA_PLOT["EXAMPLE_T_CLUSTER"][sub_pop]["T_Critical"]
t_stat = DATA_PLOT["EXAMPLE_T_CLUSTER"][sub_pop]["t_stat"]
index=DATA_PLOT["EXAMPLE_T_CLUSTER"][sub_pop]["Initial_T_Cluster_AND_Index_Info"][1]
t_n=len(DATA_PLOT["EXAMPLE_T_CLUSTER"][sub_pop]["Initial_T_Cluster_AND_Index_Info"][0])
...
ax1[0].set_ylabel("t-statistic",fontsize=label_font)
...
p_value=DATA_PLOT["EXAMPLE_T_CLUSTER"][sub_pop]["P_Value"]
k = 0
f = open(root_fig+'p_value_POSTSTIMULUS_PLOT_UNPERTURBED_CORTICAL_COLUMN_SCREENING_{}.txt'.format(sub_pop), 'w')
f.write("Panel A\n\n")
f.write(" "*20)
for i in range(t_n):
    ax_1.fill_between(index[i],[0.2 ,0.2],[-0.2 ,-0.2],zorder=2,color="k")
for i in range(len(p_value)):
    f.write("{:^10}".format(p_value[i]))
    if p_value[i] <= 0.01:
...
ax_1.set_ylim(-2,2)
ax_1.text(25000,-2.5,"t-cluster statistics",ha='center',fontsize=label_font)
ax_1.axis('off')
...
f.write(" "*20)
for b in beta:
    f.write("{:^10}".format("beta={}".format(b)))
f.write("\n\n")
for i,g in enumerate(g_ampa):
    f.write("{:^20}".format("g_ampa={}".format(g)))
    col = [None]*5
    data_aux = [None]*5
    data_aux1 = [None]*5
...
plt.savefig(root_fig+"POSTSTIMULUS_PLOT_UNPERTURBED_CORTICAL_COLUMN_SCREENING_{}.pdf".format(sub_pop),bbox_inches = 'tight', pad_inches = 0)
